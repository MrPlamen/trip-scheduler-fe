...
const Search = () => {
    ...
    useEffect(() => {
        const handler = setTimeout(() => {
            setDebouncedEmail(memberEmail.trim());
        }, 1000); // 500ms debounce

        return () => clearTimeout(handler); 
    }, [memberEmail]);

    const { filteredTrips, error } = useSearchTrip(debouncedEmail);
    const { filteredItems, errorItem } = useSearchVisitItem(debouncedEmail);
    ...
    return (
        <div className="search-container">
            ...
            {(filteredTrips.length > 0 || filteredItems.length > 0) && (
                            ...
                            {filteredTrips.length > 0 ? (
                                filteredTrips.map((trip) => {
                                    const isMember =
                                        ...
                                        trip.members.some(member => member.email === debouncedEmail);
                                    const content = (
                                        <>
                                    ...
                                    return (
                                        <li key={trip._id} className="search-item">
                                            {isMember ? (
                                                <Link to={`/trips/${trip._id}/details`} className="search-link">
                                                    {content}
                                                </Link>
                                            ) : content}
                                        </li>
                            ...
                            {filteredItems.length > 0 ? (
                                filteredItems.map((item) => {
                                    const isMember =
                                        ...
                                        item.members.some(member => member.email === debouncedEmail);
                                    const content = (
                                        <>
                                    ...
                                    return (
                                        <li key={item._id} className="search-item">
                                            {isMember ? (
                                                <Link to={`/visits/${item._id}/details`} className="search-link">
                                                    {content}
                                                </Link>
                                            ) : content}
                                        </li>
